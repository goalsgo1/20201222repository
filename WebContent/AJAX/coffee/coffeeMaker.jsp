    ...
    function createRequest(){
     ...
     function serveDrink(){
        ...
        if(request1.readyState == 4){
           if(request1.status == 200){
              //응답 받아오기 ==> 1김유신
              var res = request1.responseText;//responeXML
              var machine = res.substring(0,1);//1 커피머신 번호를 받아온다.
           ...
           if(request2.status == 200){
              ...
              if(machine == "1"){
                   var coffeemaker1 = document.getElementById("coffmaker1Status");
                   //@param:위치정보, @param:그 위치에 대체될 문자열값이다.
                   //Idle ->커피 주문 -> 님의 커피를 내리는 중입니다. -> Idle
                   replaceText(coffeemaker1, "Idle"); 
     ...
     function orderCoffee(){
        ...
        if(status=="Idle"){
           ...
           var url = "coffeeMade.jsp?coffeemaker=1&name="+name+"&timestamp="+new Date().getTime();
           ...
           if(status=="Idle"){
              ...
              //두번째 주문이 처리가 된 경우임.
              document.forms[0].reset();
              var url = "coffeeMade.jsp?coffeemaker=2&name="+name+"&timestamp="+new Date().getTime();
